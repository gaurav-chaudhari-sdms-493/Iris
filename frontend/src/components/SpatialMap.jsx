...
export default function SpatialMap({ zoneStates, relays, tvState, acState, systemMode, onToggleSwitch, onSetMode }) {
  ...
  const renderSquarePanel = (id, isOn, switchId) => (
    ...
    >
      <div className={`relative w-16 h-14 rounded-xl border-2 transition-all duration-300 flex flex-col items-center justify-center p-1 ${isOn
        ...
        <span className={`text-xs font-mono font-black tracking-wider ${isOn ? 'text-slate-950' : 'text-slate-200'}`}>
          {id}
        </span>
        <span className={`text-[9px] font-mono font-bold ${isOn ? 'text-slate-900 font-extrabold' : 'text-slate-500'}`}>
          {isOn ? 'LIT' : 'OFF'}
        </span>
      </div>
    </div>
  ...
  return (
    <div className="iris-card flex flex-col justify-between h-full space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 pb-3">
        ...
        <div className="flex flex-wrap items-center gap-2 bg-slate-950 p-1.5 rounded-2xl border border-slate-800">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl font-extrabold text-xs flex items-center gap-1.5 border transition-all cursor-pointer active:scale-95 ${activePreset === 'AI_MODE'
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl font-extrabold text-xs flex items-center gap-1.5 border transition-all cursor-pointer active:scale-95 ${activePreset === 'ALL_ON'
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl font-extrabold text-xs flex items-center gap-1.5 border transition-all cursor-pointer active:scale-95 ${activePreset === 'ALL_OFF'
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl font-extrabold text-xs flex items-center gap-1.5 border transition-all cursor-pointer active:scale-95 ${activePreset === 'PANELS_ONLY'
      ...
      <div className="relative flex-1 flex flex-col justify-between bg-slate-950 border-2 border-slate-800 rounded-3xl p-3 md:p-3.5 space-y-2 shadow-2xl bg-[linear-gradient(to_right,#1e293b20_1px,transparent_1px),linear-gradient(to_bottom,#1e293b20_1px,transparent_1px)] bg-[size:24px_24px]">
        ...
        <div className="grid grid-cols-3 gap-2 items-center justify-items-center relative">
          {renderSquarePanel('LP2', isS3On, 'S3')}

          {/* SQUARE CASSETTE AC UNIT */}
          <div className={`w-16 h-14 rounded-xl border-2 transition-all flex flex-col items-center justify-center p-1 relative shadow-xl z-10 ${isAcOn
            ? 'bg-slate-950 border-emerald-500 text-emerald-300 shadow-[0_0_20px_rgba(16,185,129,0.4)]'
            : 'bg-slate-900/90 border-slate-800 text-slate-500'
            }`}>
            {/* Louvers */}
            <div className="absolute top-1 left-1.5 right-1.5 h-0.5 bg-slate-700 rounded" />
            <div className="absolute bottom-1 left-1.5 right-1.5 h-0.5 bg-slate-700 rounded" />

            <div className="flex items-center gap-1 text-[11px] font-black tracking-wide">
              <Wind className={`w-3.5 h-3.5 ${isAcOn ? 'text-emerald-400 animate-spin' : 'text-slate-600'}`} />
              <span>AC</span>
            </div>
            <span className={`text-[9px] font-extrabold px-1 rounded-full border ${isAcOn
